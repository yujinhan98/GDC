...
args.save = args.save_path + args.dataset + '_metric'
...
torch.cuda.manual_seed(args.seed)
import re
model_loads_text = re.search(r'label(.*?)epoch', args.sload)
...
def embbeding(data_loader_img,name):
    emb_dir = 'emb_data/'+'text'+str(model_loads_text )+'_seed'+str(args.seed)+'_'+name+'_celebA_emb_data'
...
dim = train_loader_emb.dataset[0][0].view(-1).shape[0]#train_loader_emb.dataset[0][0].shape[0]
...
train_accuracy, train_saccuracy, train_slabels, train_labels,spu_train_labels= test_disk(train_loader_emb)
eval_accuracy, eval_saccuracy,eval_slabels,eval_labels,spu_eval_labels  = test_disk(eval_loader_emb)
test_accuracy, test_saccuracy,test_slabels,test_labels,spu_test_labels = test_disk(test_loader_emb)
...
# if args.dataset
# worst_loader_train = DataLoader(Subset(train_loader_img.dataset, worst_indices_train), batch_size=args.batch_size, shuffle=False, pin_memory=True,num_workers=16)
# worst_loader_eval = DataLoader(Subset(eval_loader_img.dataset, worst_indices_eval), batch_size=args.batch_size, shuffle=False, pin_memory=True,num_workers=16)
# worst_loader_test = DataLoader(Subset(test_loader_img.dataset, worst_indices_test), batch_size=args.batch_size, shuffle=False, pin_memory=True,num_workers=16)
# worst_loader_test = DataLoader(worst_dataset, batch_size= args.batch_size, shuffle=False)   
...
def recall_precision(labels,slabels,spu_labels,worst_indices):
    if args.dataset == 'cmnist':
        ...
        precision = len(correct) / len(pre_minority) * 100
        recall = len(correct) / len(worst_indices) * 100
        print(f"The Group is :{key[0]},{key[1]} | Precision-Percentage of common elements in predicted_minority_group: {precision}%")
        print(f"The Group is :{key[0]},{key[1]} | Recall-Percentage of common elements in minority_group: {recall}%")
        with open(os.path.join(args.save, args.dataset+ '_s' + str(args.seed)+ '_smodel'+str(model_loads_text)+'_metric.csv'), 'a') as f:
            f.write('The Group is :(%.1f,%.1f) | Minority Group Precision: %0.4f\n'%(key[0],key[1],precision))
            f.write('The Group is :(%.1f,%.1f) | Minority Group Recall: %0.4f\n'%(key[0],key[1],recall))
    #     # break
...
# print('Beginning Testing DISK on Minority Groups\n')
# train_loader_emb =  embbeding(worst_loader_train,'train')
# eval_loader_emb =  embbeding(worst_loader_eval,'eval')
# test_loader_emb =  embbeding(worst_loader_test,'test')
# with open(os.path.join(args.save, args.dataset+ '_s' + str(args.seed)+ '_smodel'+str(model_loads_text)+'_metric.csv'), 'a') as f:
#     f.write('Minority Group | DISK: train_accuracy,eval_accuracy,test_accuracy\n')
# train_accuracy, train_saccuracy, train_slabels, train_labels,spu_train_labels= test_disk(train_loader_emb)
# eval_accuracy, eval_saccuracy,eval_slabels,eval_labels,spu_eval_labels  = test_disk(eval_loader_emb)
# test_accuracy, test_saccuracy,test_slabels,test_labels,spu_test_labels = test_disk(test_loader_emb)
# print('Minority Group | Train Accuracy: %.4f | Eval Accuracy: %.4f | Test Accuracy: %.4f' % (train_accuracy,eval_accuracy,test_accuracy))
# print('Minority Group | Train SAccuracy: %.4f | Eval SAccuracy: %.4f | Test SAccuracy: %.4f' % (train_saccuracy,eval_saccuracy,test_saccuracy))
# with open(os.path.join(args.save, args.dataset+ '_s' + str(args.seed)+ '_smodel'+str(model_loads_text)+'_metric.csv'), 'a') as f:
#     f.write('%0.4f,%0.4f,%0.4f\n' % (train_accuracy,eval_accuracy,test_accuracy))

# with open(os.path.join(args.save, args.dataset+ '_s' + str(args.seed)+ '_smodel'+str(model_loads_text)+'_metric.csv'), 'a') as f:
#     f.write('Minority Group | DISK: train_saccuracy,eval_saccuracy,test_saccuracy\n')
