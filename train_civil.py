...
parser.add_argument('--disk_epochs', type=int, default=5, help='Number of epochs to train disk.')
...
parser.add_argument('--eval_weight', type=int, default=2, help='Loss weight of disk')
...
if args.model_type == 'r50': #r50:
    ...
    # state_dict = torch.load('/home/yujin/dm/disk/snapshots/civilcommentsbert_epoch5_lr1e-05_estrain/civilcomments_classifier_depoch12_dlr1e-05_labelTrue_hcsNone_s0_upweight_best_epoch.pt', map_location=device)
    # net = bert_pretrained(output_dim=2).to(device)  
...
# /////////////// Training ///////////////
criterion_train = torch.nn.CrossEntropyLoss() #nn.BCELoss()#torch.nn.CrossEntropyLoss()
...
#     for images, labels, slabels in tqdm(data_loader_img):
#         images, labels=images.to(device),labels.to(device)
...
train_accuracy, train_saccuracy, train_slabels, train_labels= test_disk(train_loader_emb)
eval_accuracy, eval_saccuracy,eval_slabels,eval_labels = test_disk(eval_loader_emb)
test_accuracy, test_saccuracy,_,_ = test_disk(test_loader_emb)
print('Train Accuracy: %.4f | Eval Accuracy: %.4f | Test Accuracy: %.4f' % (train_accuracy,eval_accuracy,test_accuracy))
...
# elif args.mode == 'jtt':
#     # min_samples = min(len(indices) for indices in indices_dict.values())
#     min_samples = sorted([len(indices) for indices in indices_dict.values()])[::-1][1]
#     balanced_indices = []
#     for key, indices in indices_dict.items():
#         if len(indices) <= min_samples:
#             ratio = args.lambdas
...
def load_group_data(identity):
    train_dataset_img, eval_dataset_img,test_dataset_img = load_civilcomments(spurious_labels = identity)
    train_loader_img = DataLoader(train_dataset_img, batch_size=16, shuffle=False, pin_memory=True)
    new_subset = Subset(train_loader_img.dataset, indices=[i for i in range(len(train_loader_img.dataset)) if i != 141037])
    train_loader_img = DataLoader(new_subset, batch_size=16, shuffle=False, num_workers=train_loader_img.num_workers, pin_memory=train_loader_img.pin_memory)
    eval_loader_img = DataLoader(eval_dataset_img, batch_size=16, shuffle=False, pin_memory=True)
    test_loader_img = DataLoader(test_dataset_img, batch_size=16, shuffle=False, pin_memory=True)
    new_subset = Subset(test_loader_img.dataset, indices=[i for i in range(len(test_loader_img.dataset)) if i != 73529])
    test_loader_img = DataLoader(new_subset, batch_size=test_loader_img.batch_size, shuffle=False, num_workers=test_loader_img.num_workers, pin_memory=test_loader_img.pin_memory)
    return train_loader_img,eval_loader_img ,test_loader_img

def test_worst_group(values,test_loader_img,ids):
    worst_indices_test = [idx for idx, (_, label, slabel) in enumerate(test_loader_img.dataset) if label == values[0] and slabel == values[1]]
    ...
    with torch.no_grad():
        for data, target,_ in worst_loader:
            ...
            correct += (preds == target).sum().item()
    accuracy = correct / len(worst_loader.dataset)
    print('The (%s) Group is :(%.1f,%.1f) | Test Worst Accuracy: %.4f ' % (ids,values[0],values[1],accuracy))
    with open(os.path.join(args.save, args.dataset + '_DISK' + '_depoch' + str(args.disk_epochs)+'_dlr'+str(args.disk_lr )+'_label'+ str(args.use_label) +'_hcs'+str(args.hcs)+ '_s' + str(args.seed) + '_' + args.mode +str(args.upweight_factor)+ '_training_results.csv'), 'a') as f:
         f.write('The (%s) Group is :(%.1f,%.1f) | Test Worst Group Accuracy: %0.4f\n' % (ids,values[0],values[1],accuracy))
    return accuracy
...
if args.mode in  ['upweight','downweight','mixweight','jtt']:
    ...
    with open(os.path.join(args.save, args.dataset + '_DISK' + '_depoch' + str(args.disk_epochs)+'_dlr'+str(args.disk_lr )+'_label'+ str(args.use_label) +'_hcs'+str(args.hcs)+ '_s' + str(args.seed) + '_' + args.mode +str(args.upweight_factor)+ '_training_results.csv'), 'a') as f:
        f.write('\nepoch,train_loss, train ACC, min ACC\n')
    train(upsampled_train_loader_img,min_loader_img)
    with open(os.path.join(args.save, args.dataset + '_DISK' + '_depoch' + str(args.disk_epochs)+'_dlr'+str(args.disk_lr )+'_label'+ str(args.use_label) +'_hcs'+str(args.hcs)+ '_s' + str(args.seed) + '_' + args.mode +str(args.upweight_factor)+ '_training_results.csv'), 'a') as f:
        f.write('Fianl Results: \n')
    test()
    for ids in ["male", "female", "LGBTQ", "christian", "muslim", 'other_religions', "black", "white"]:
        train_loader_img,eval_loader_img ,test_loader_img = load_group_data(identity=ids)
        test_worst_group([0,0],test_loader_img,ids)
        test_worst_group([1,0],test_loader_img,ids)
        test_worst_group([0,1],test_loader_img,ids)
        test_worst_group([1,1],test_loader_img,ids)
    # test_worst([0,0])
    # test_worst([1,0])
    # test_worst([0,1])
    # test_worst([1,1])
    ...
    net = model_factory("distilbert", (0,0,0), trainset.num_classes).to(device)
    optimizer_dro = bert_adamw_optimizer(net, lr=args.lr, weight_decay=args.decay)
